...
Он сразу доступен после `from drawzero import *`.
...
Прочтите [Примеры](examples.md) и откройте `05_points.py` или `09_animation_rectangles.py`, чтобы увидеть более длинные скрипты, которые используют эти методы вместе.
...
* [Примеры](examples.md) – таблица скриптов, включая те, которые сосредоточены на `Pt`.
